...
- **Analytics**: Google tag (gtag.js) snippet is inlined in every page `<head>` — preserve the tag ID
  (`GT-5MCNDKRF`) when copying/editing head markup. This is a unified Google tag container, not a bare
  Google Ads conversion ID — it fans out to linked destinations (Google Ads, GA4, etc.) configured in
  Google's backend, not in this repo. The homepage's enquiry form (and each `locations/*.html` copy of it)
  fires a `generate_lead` custom event via `gtag()` on successful submission — keep that event name in sync
  with whatever conversion action is configured against it in Google Ads.
